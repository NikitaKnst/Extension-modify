...
function TaimerFunction() {/*Таймер срабатывание */
  setInterval(function (){ console.log("3 sec end"); ChoiseWord() }, 3000);/*Последние число кол-во миллисекунд */ 
}
function tes(){
  console.log("Proslo")/*Тест*/
}
async function  ChoiseWord(){/*Выбираем слова и разделяем их на строки*/
  var b= document.getElementById("textAreaSecond").value/*Получаем значение с textarea*/
      ...
      Array.prototype.forEach.call(SrchWord,function(element,index){
             /* const parent = this.el.parentElement; урааааа как как??!?!
             Array.prototype.forEach.call(parent.children,function(element,index){*/

         console.log("3")/*NEEEEEEEEEEEEEEEEEEET*/
      ...
      if (list[i] == SrchWord){/*Если находим*/
        ...
        console.log("Ничего не найдёно")/*рисуем*/
      }
    }
})
  return [stringWords, words];/*взрщем*/
}
